...
const Result = () => {
    ...
    useEffect(() => {
        ...
        setIsloading(true)
        axios
            .post(process.env.NEXT_PUBLIC_URL + '/admin/result', data)
            .then((response) => {
        ...
        if(JSON.stringify(editedDoc) != "{}")
            for(let idx in data)
                if(data[idx]._id == editedDoc._id) {
                    axios.put(process.env.NEXT_PUBLIC_URL + '/admin/result/update', editedDoc)
                        .then(response => {
                            data[idx] = {...editedDoc}
                            setData([...data])
                        }).catch(err => console.log(err.message))
    ...
    <>
        <div className="mr-2 flex justify-between">
            ...
            <div className="flex mt-2 space-x-2">
                <Upload url={process.env.NEXT_PUBLIC_URL + '/admin/result/upload'}/>
                {data ? 
               
                <Download url={process.env.NEXT_PUBLIC_URL + '/admin/result/download'} ids={data.filter(doc => filterCheck(doc)).map(doc => doc._id)} name="results_downlad" />
