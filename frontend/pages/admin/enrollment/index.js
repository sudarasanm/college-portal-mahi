...
//  }, []);
 ...
 const deleteStudent = (courseCode, ind) => (e) => {
  ...
  const url = process.env.NEXT_PUBLIC_URL + '/admin/enrolment/removestudents';
      axios
...
const handleOKClick = (addrow) => {
  ...
  const url = process.env.NEXT_PUBLIC_URL + '/admin/enrolment/addstudents';
      axios
        .post(url, addstudent)
        .then((res) => {
...
const handleInput = (e) => {
  ...
  if(enrol === true){
    ...
    const saveEnrol = () => {
      setLoading(true);
      const url = process.env.NEXT_PUBLIC_URL + '/admin/enrolment/getdata';
      axios
    ...
    const saveApprove = () => {
      setLoading1(true);
      const url = process.env.NEXT_PUBLIC_URL + '/admin/enrolment/approve';
      axios
        .post(url, approve)
        .then((res) => {
          if(res.data.success){
